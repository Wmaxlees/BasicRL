...
@partial(jax.jit, static_argnames=('agent_params'))
def update_q_table(agent_state: AgentState, s: jnp.ndarray, a: int, r: float, s_prime: jnp.ndarray, a_prime: int, agent_params: AgentParams):
    new_q_table = agent_state.q_table.at[s, a].set(
        agent_state.q_table[s, a] + agent_params.alpha * (r + agent_params.gamma * agent_state.q_table[s_prime, a_prime] - agent_state.q_table[s, a])
    )
...
def main():
    ...
    for i in range(1000):
        ...
        for _ in range(1000):
            ...
            key, n_action = choose_action(key, agent_state, n_obs, agent_params)

            agent_state = update_q_table(agent_state, obs, action, reward, n_obs, n_action, agent_params)
